...
{
    if (only_types)
        actions->add(ExpressionAction::ordinaryJoin(join_params, nullptr, analyzedJoin().key_names_left, columns_added_by_join));
    else
        for (auto & subquery_for_set : subqueries_for_sets)
            if (subquery_for_set.second.join)
                actions->add(ExpressionAction::ordinaryJoin(join_params, subquery_for_set.second.join, analyzedJoin().key_names_left,
